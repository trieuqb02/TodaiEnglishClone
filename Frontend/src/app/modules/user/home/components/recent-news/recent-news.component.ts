...
export class RecentNewsComponent implements OnInit, OnChanges {
  @Input() showInputDate:boolean = true;
  @Input() search!:string;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['search']) {
      console.log('Search value đã thay đổi:', changes['search'].currentValue);
      // call api
    }
    if (changes['topicId']) {
      console.log('Topic ID đã thay đổi:', changes['topicId'].currentValue);
      // call api
    }
    if (changes['sourceId']) {
      console.log('Source ID đã thay đổi:', changes['sourceId'].currentValue);
      // call api
    }
  }

  public concatParagraphs(paragraphs: ParagraphModel[]): string {
    return paragraphs.reduce((init: string, currentValue: ParagraphModel):string => {
      return init + ' ' + currentValue.content;
    }, '')
  }
